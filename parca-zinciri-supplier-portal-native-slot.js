...
`;
  ...
  class ParcaZinciriSupplierPortal extends HTMLElement {
    ...
    _render() {
      ...
      if (productForm && nextProductForm) nextProductForm.replaceWith(productForm);
      this._renderToast();
    ...
    _renderWebProductPage() {
      ...
      return '<section class="panel product-page" aria-labelledby="web-product-title"><div class="panel-h"><h2 id="web-product-title" class="h2" tabindex="-1" data-autofocus>Yeni Parça Ekle</h2><button type="button" class="btn" data-action="leave-product-add">← Geri dön</button></div><form id="pz-web-product-form">'+
        ...
        '<div class="grid-2">'+input('productCode','Ürün / stok kodu','text','maxlength="80"')+input('stockQuantity','Stok adedi','number','min="1" step="1"')+input('priceEur','Birim fiyat (Euro / EUR)','number','min="0.01" step="0.01"')+'</div><label class="check"><input name="productCodeUnknown" type="checkbox">Diğer — ürün / stok kodu elimde yok</label>'+
