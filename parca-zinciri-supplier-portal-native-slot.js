...
`;
  ...
  class ParcaZinciriSupplierPortal extends HTMLElement {
    ...
    _setRoute(route) {
      if (this._webSaving) { this._toast("Kayıt sürüyor", "Lütfen parça kaydının tamamlanmasını bekleyin."); return; }
    ...
    _onClick(e) {
      ...
      if (action === "add-inventory") {
        ...
        this._webProductReturnRoute = s.route === "inventory" ? "inventory" : "overview";
        this._setRoute("product-add");
        this._loadWebProductCatalog();
        return;
      }
      if (action === "edit-rejected-product") { this._openRejectedProduct(id); return; }
    ...
    _render() {
      var s = this._state;
      var productForm = s.screen === "app" && s.route === "product-add" && this._root.querySelector("#pz-web-product-form");
      this._ensureHostFillStyles();
      ...
      var nextProductForm = this._root.querySelector("#pz-web-product-form");
      if (productForm && nextProductForm) {nextProductForm.replaceWith(productForm);this._syncWebProductFields(productForm);}
      this._renderToast();
    ...
    _renderRoute() {
      switch (this._state.route) {
        ...
        case "product-add":
          return this._renderWebProductPage();
        case "orders":
    ...
    _renderLiveInventory() {
      ...
      var content=rows.map(function(p){
        ...
        return '<article class="panel" data-live-listing="'+esc(p.listingKey)+'"><h3>'+esc(p.title)+'</h3><div>'+media+'</div><p>'+esc(['machine','workshop'].includes(p.listingType)?(p.machineSerialNumber?'Seri no: '+p.machineSerialNumber+' · ':''):(p.productCodeUnknown?'Parça no belirtilmedi · ':'Parça kodu: '+(p.productCode||'Belirtilmedi')+' · '))+esc(p.stockQuantity)+' adet · '+esc(money(p.priceEur,'EUR'))+'</p><p>'+esc(status[p.status]||p.status)+'</p>'+(p.status==='rejected'?'<p><strong>Ret gerekçesi:</strong> '+esc(p.rejectionReason||'Gerekçe belirtilmemiş.')+'</p><button type="button" class="btn primary sm" data-action="edit-rejected-product" data-id="'+esc(p.listingKey)+'">Düzenle ve yeniden gönder</button>':'')+'<details><summary>Kayıt bilgileri</summary><p>Satıcı no: '+esc(p.sellerNumber)+'</p><p>Kayıt no: '+esc(p.listingKey)+'</p><p>Kullanıcı no: '+esc(p.ownerMemberId)+'</p></details></article>';
      }).join('');
    ...
    async _openRejectedProduct(listingKey) {
      if(this._webSaving)return;
      const scope=this._inventoryScope, request=crypto.randomUUID();
      this._webEditRequest=request;
      this._toast('Ürün yükleniyor','Mevcut bilgiler ve fotoğraflar alınıyor.');
      try {
        const row=await this._webCall('product',{listingKey});
        if(scope!==this._inventoryScope || request!==this._webEditRequest || this._state.route!=='inventory')return;
        if(row.status!=='rejected')throw Error('Bu ürün artık reddedildi durumunda değil. Listeyi yenileyin.');
        if(!['part','equipment','machine','workshop'].includes(row.listingType))throw Error('Bu eski kayıt için destek alın; bilgileriniz korunuyor.');
        this._webProductReturnRoute='inventory';
        this._setRoute('product-add');
        const form=this._root.querySelector('#pz-web-product-form'),fields=form.elements;
        form._editing=row;
        fields.listingType.value=row.listingType;
        for(const name of ['partName','partOriginType','partCondition','equipmentType','equipmentCondition','equipmentWorkDescription','machineType','machineSerialNumber','modelYear','machineModificationSummary','productCode','stockQuantity','priceEur','description']) {
          if(row[name]!=null && row[name]!=='')fields[name].value=row[name];
        }
        fields.materialType.value=row.listingType==='workshop'?row.title:'';
        fields.partManufacturer.value=row.partManufacturer||'';fields.partGroupId.value=row.partGroupId||'';this._syncWebProductFields(form,'partGroupId');fields.partSubgroupId.value=row.partSubgroupId||'';
        fields.workshopCondition.value=row.condition==='used'?'used':'new';
        fields.productCodeUnknown.checked=row.productCodeUnknown===true;
        const rejection=form.querySelector('[data-edit-rejection]');
        rejection.hidden=false;rejection.textContent='Ret gerekçesi: '+(row.rejectionReason||'Gerekçe belirtilmemiş.');
        const media=row.media||[],photos=media.filter(m=>String(m.mime).startsWith('image/'));
        form._retainedVideos=media.filter(m=>String(m.mime).startsWith('video/')).map(m=>m.id||m.fileId);
        photos.slice(0,6).forEach((photo,i)=>{
          const card=form.querySelector('[data-photo-slot="'+i+'"]'),img=card.querySelector('img');
          card._existingPhoto=photo;card.classList.add('has-photo');
          if(/^https:\/\/(static\.wixstatic\.com|video\.wixstatic\.com|[a-z0-9.-]+\.wixmp\.com)\//i.test(photo.url||'')){img.src=photo.url;img.hidden=false;}
          card.querySelector('[data-photo-caption]').textContent='Fotoğrafı değiştir';
          card.querySelector('[data-photo-status]').textContent='✓ Kayıtlı fotoğraf';
        ...
        if(row.listingType==='machine') {
          ...
          await this._loadWebProductCatalog();
          if(scope!==this._inventoryScope || !form.isConnected)return;
          fields.machineBrandId.value=row.machineBrandId||'';
          this._syncWebProductFields(form,'machineBrandId');
          fields.machineModelId.value=row.machineModelId||'';
          this._syncWebProductFields(form);
          fields.manualBrandName.value=row.manualBrandName||'';fields.manualModelName.value=row.manualModelName||'';
          form.querySelector('[data-product-fields]').disabled=false;fields.listingType.disabled=false;
          form._loading=false;form.querySelector('[type="submit"]').disabled=false;
        }
      } catch(e){if(scope===this._inventoryScope && request===this._webEditRequest)this._toast('Ürün açılamadı',e.message);}
    }
    ...
    async _loadWebProductCatalog() {
      const scope = this._inventoryScope;
      const form = this._root.querySelector('#pz-web-product-form');
      if (!form) return;
      const status = form.querySelector('[data-catalog-status]');
      status.textContent = 'Makine kataloğu yükleniyor…';
      try {
        const catalog = await this._webCall('catalog');
        if (!catalog?.brandsByType || !catalog?.modelsByTypeAndBrand || !catalog.otherBrandId || !catalog.otherModelId) throw Error('Makine kataloğu alınamadı.');
        if (scope !== this._inventoryScope || !form.isConnected) return;
        this._webMachineCatalog = catalog;
    ...
    _syncWebProductFields(form, changed) {
      ...
      const partGroups=[{"id":"drivetrain","label":"Aktarma Grubu","children":[{"id":"tie_rod","label":"Rot Başları"},{"id":"shaft_yoke","label":"Şaft Yokeleri"},{"id":"joint","label":"Şaft Mafsalı"},{"id":"shaft_bolt","label":"Şaft Kelepçe Civataları"},{"id":"shaft_housing","label":"Şaft Housing"},{"id":"cross_yoke","label":"Komple İstavroz Çatalı"},{"id":"support_bearing","label":"Askı Bilyası"}]},{"id":"fasteners","label":"Civata ve Pimler","children":[{"id":"bolt","label":"Civata Çeşitleri"}]},{"id":"engine","label":"Motor Yedek Parçaları","children":[{"id":"exhaust","label":"Egzoz"},{"id":"piston","label":"Piston, Gömlek, Segman"},{"id":"head","label":"Motor Silindir Kapağı"},{"id":"fan","label":"Motor Pervanesi"},{"id":"compressor","label":"Kompresör"},{"id":"camshaft","label":"Eksantrik Mili"},{"id":"turbo","label":"Komple Turbo"},{"id":"block","label":"Motor Blokları"},{"id":"crank","label":"Motor Krank Mili"},{"id":"overhaul","label":"Motor Revizyon Parçaları"},{"id":"complete_engine","label":"Komple Motor"}]},{"id":"pump","label":"Pompa ve Pompa Ekipmanları","children":[{"id":"oil_pump","label":"Yağ Pompası"},{"id":"pump_parts","label":"Pompa Parçaları"},{"id":"cartridge","label":"Pompa Kartuşları"},{"id":"barrel","label":"Barrel"},{"id":"water_pump","label":"Devirdaim Pompası"},{"id":"hydraulic_pump","label":"Hidrolik Pompa"},{"id":"injection","label":"Mazot Pompa Enjektör Memesi"},{"id":"hydraulic_cylinder","label":"Hidrolik Silindir"}]},{"id":"bearing","label":"Rulmanlar","children":[{"id":"bearing_types","label":"Rulman Çeşitleri"}]},{"id":"transmission","label":"Şanzıman ve Dişli Grubu","children":[{"id":"turn_hub","label":"Dönüş Hupu"},{"id":"double_pinion","label":"Çiftli Pinyon"},{"id":"single_pinion","label":"Tekli Pinyon"},{"id":"final_gear","label":"Cer Dişlileri"},{"id":"torque_gear","label":"Tork Dişlisi"},{"id":"diff_gears","label":"Diferansiyel Dişli Grubu"},{"id":"clutch","label":"Şanzıman Disk ve Balataları"},{"id":"gears","label":"Dişli Çeşitleri"},{"id":"torque_cover","label":"Tork Kapağı"},{"id":"complete_transmission","label":"Komple Şanzıman"},{"id":"front_diff","label":"Ön Diferansiyel"},{"id":"rear_diff","label":"Arka Diferansiyel"}]},{"id":"chassis","label":"Şase Aksamı","children":[{"id":"strip","label":"Strip"},{"id":"operator_cabin","label":"Operatör Kabini"},{"id":"complete_chassis","label":"Şase"}]},{"id":"sealing","label":"Sızdırmazlık Ekipmanları","children":[{"id":"head_gasket","label":"Motor Silindir Kapak Contası"},{"id":"hydraulic_seal","label":"Hidrolik Conta Seti"},{"id":"oring_set","label":"O-Ring Seti"},{"id":"gasket_set","label":"Conta Takımları"}]},{"id":"wear","label":"Tırnak, Bıçak, Kazıcı ve Delici Uçlar","children":[{"id":"ripper","label":"Dozer Riper Adaptörü"},{"id":"loader_edge","label":"Loder Ağız Bıçakları"},{"id":"bucket_edge","label":"Kepçe Önleri"},{"id":"grader_blade","label":"Greyder Bıçakları"},{"id":"middle_blade","label":"Dozer Orta Bıçak"},{"id":"dozer_blade","label":"Dozer Bıçakları, Tırnak ve Adaptörler"},{"id":"blade","label":"Bıçak Çeşitleri"},{"id":"smk_wa420","label":"SMK-WA420 Kazma"},{"id":"smk","label":"SMK-Kazma"}]},{"id":"undercarriage","label":"Yürüyüş Takımları","children":[{"id":"carrier","label":"Taşıyıcı Makara"},{"id":"sprocket","label":"Rim Dişlisi"},{"id":"roller","label":"Yürüyüş Makarası"},{"id":"idler","label":"İstikamet Tekeri"},{"id":"chain","label":"Palet Zinciri"},{"id":"shoe","label":"Palet Pabucu"},{"id":"track","label":"Palet"},{"id":"link","label":"Bakla, Pim, Burç"}]}];
      if(fields.partGroupId && changed==='partGroupId'){const g=partGroups.find(g=>g.id===fields.partGroupId.value);fields.partSubgroupId.innerHTML='<option value="">Alt grup seçin</option>'+(g?g.children:[]).map(c=>'<option value="'+esc(c.id)+'">'+esc(c.label)+'</option>').join('');}
      ...
      const codeOff = isMachine || kind === 'workshop' || fields.productCodeUnknown.checked;
      ...
      const noun = {part:'Parça',equipment:'Ekipman',machine:'Makine',workshop:'Atölye Malzemesi'}[kind];
    ...
    _renderWebProductPage() {
      ...
      return '<section class="panel product-page" aria-labelledby="web-product-title"><div class="panel-h"><h2 id="web-product-title" class="h2" tabindex="-1" data-autofocus>Yeni Parça Ekle</h2><button type="button" class="btn" data-action="leave-product-add">← Geri dön</button></div><form id="pz-web-product-form"><fieldset data-product-fields>'+
        '<h3>Ürün bilgileri</h3>'+select('listingType','İlan türü',[['part','Parça'],['equipment','Ekipman'],['machine','Makine'],['workshop','Atölye Malzemeleri (Torna, Kompresör, Takım, Pres vb.)']])+
        '<p data-edit-rejection role="note" hidden></p><div class="grid-2" data-product-kind="workshop" hidden>'+input('materialType','Malzeme türü','text','maxlength="200" placeholder="Örneğin torna, kompresör, takım veya pres" disabled')+select('workshopCondition','Durum',[['new','Yeni'],['used','2. el']],'disabled')+'</div><div class="grid-2" data-product-kind="part">'+input('partName','Parça adı','text','maxlength="200"')+
        ...
        '<div data-product-kind="part equipment"><input type="hidden" name="partsTaxonomyVersion" value="2"><div class="grid-2">'+input('partManufacturer','Parça üreticisi','text','maxlength="80" placeholder="Örn. ZF, Perkins, CAT"')+'</div></div>'+
        '<div class="grid-2" data-product-kind="part">'+select('partGroupId','Parça grubu',[['','Grup seçin'],...[{"id":"drivetrain","label":"Aktarma Grubu","children":[{"id":"tie_rod","label":"Rot Başları"},{"id":"shaft_yoke","label":"Şaft Yokeleri"},{"id":"joint","label":"Şaft Mafsalı"},{"id":"shaft_bolt","label":"Şaft Kelepçe Civataları"},{"id":"shaft_housing","label":"Şaft Housing"},{"id":"cross_yoke","label":"Komple İstavroz Çatalı"},{"id":"support_bearing","label":"Askı Bilyası"}]},{"id":"fasteners","label":"Civata ve Pimler","children":[{"id":"bolt","label":"Civata Çeşitleri"}]},{"id":"engine","label":"Motor Yedek Parçaları","children":[{"id":"exhaust","label":"Egzoz"},{"id":"piston","label":"Piston, Gömlek, Segman"},{"id":"head","label":"Motor Silindir Kapağı"},{"id":"fan","label":"Motor Pervanesi"},{"id":"compressor","label":"Kompresör"},{"id":"camshaft","label":"Eksantrik Mili"},{"id":"turbo","label":"Komple Turbo"},{"id":"block","label":"Motor Blokları"},{"id":"crank","label":"Motor Krank Mili"},{"id":"overhaul","label":"Motor Revizyon Parçaları"},{"id":"complete_engine","label":"Komple Motor"}]},{"id":"pump","label":"Pompa ve Pompa Ekipmanları","children":[{"id":"oil_pump","label":"Yağ Pompası"},{"id":"pump_parts","label":"Pompa Parçaları"},{"id":"cartridge","label":"Pompa Kartuşları"},{"id":"barrel","label":"Barrel"},{"id":"water_pump","label":"Devirdaim Pompası"},{"id":"hydraulic_pump","label":"Hidrolik Pompa"},{"id":"injection","label":"Mazot Pompa Enjektör Memesi"},{"id":"hydraulic_cylinder","label":"Hidrolik Silindir"}]},{"id":"bearing","label":"Rulmanlar","children":[{"id":"bearing_types","label":"Rulman Çeşitleri"}]},{"id":"transmission","label":"Şanzıman ve Dişli Grubu","children":[{"id":"turn_hub","label":"Dönüş Hupu"},{"id":"double_pinion","label":"Çiftli Pinyon"},{"id":"single_pinion","label":"Tekli Pinyon"},{"id":"final_gear","label":"Cer Dişlileri"},{"id":"torque_gear","label":"Tork Dişlisi"},{"id":"diff_gears","label":"Diferansiyel Dişli Grubu"},{"id":"clutch","label":"Şanzıman Disk ve Balataları"},{"id":"gears","label":"Dişli Çeşitleri"},{"id":"torque_cover","label":"Tork Kapağı"},{"id":"complete_transmission","label":"Komple Şanzıman"},{"id":"front_diff","label":"Ön Diferansiyel"},{"id":"rear_diff","label":"Arka Diferansiyel"}]},{"id":"chassis","label":"Şase Aksamı","children":[{"id":"strip","label":"Strip"},{"id":"operator_cabin","label":"Operatör Kabini"},{"id":"complete_chassis","label":"Şase"}]},{"id":"sealing","label":"Sızdırmazlık Ekipmanları","children":[{"id":"head_gasket","label":"Motor Silindir Kapak Contası"},{"id":"hydraulic_seal","label":"Hidrolik Conta Seti"},{"id":"oring_set","label":"O-Ring Seti"},{"id":"gasket_set","label":"Conta Takımları"}]},{"id":"wear","label":"Tırnak, Bıçak, Kazıcı ve Delici Uçlar","children":[{"id":"ripper","label":"Dozer Riper Adaptörü"},{"id":"loader_edge","label":"Loder Ağız Bıçakları"},{"id":"bucket_edge","label":"Kepçe Önleri"},{"id":"grader_blade","label":"Greyder Bıçakları"},{"id":"middle_blade","label":"Dozer Orta Bıçak"},{"id":"dozer_blade","label":"Dozer Bıçakları, Tırnak ve Adaptörler"},{"id":"blade","label":"Bıçak Çeşitleri"},{"id":"smk_wa420","label":"SMK-WA420 Kazma"},{"id":"smk","label":"SMK-Kazma"}]},{"id":"undercarriage","label":"Yürüyüş Takımları","children":[{"id":"carrier","label":"Taşıyıcı Makara"},{"id":"sprocket","label":"Rim Dişlisi"},{"id":"roller","label":"Yürüyüş Makarası"},{"id":"idler","label":"İstikamet Tekeri"},{"id":"chain","label":"Palet Zinciri"},{"id":"shoe","label":"Palet Pabucu"},{"id":"track","label":"Palet"},{"id":"link","label":"Bakla, Pim, Burç"}]}].map(g=>[g.id,g.label])])+select('partSubgroupId','Alt grup',[['','Önce parça grubunu seçin']])+'</div>'+
    ...
    async _saveWebProduct(form) {
      if(this._webSaving || form._loading)return;
      const values={...Object.fromEntries(new FormData(form)),productCodeUnknown:['machine','workshop'].includes(form.elements.listingType.value) || form.elements.productCodeUnknown.checked}, cards=Array.from(form.querySelectorAll('[data-photo-slot]')).filter(c=>c._photoFile||c._existingPhoto), files=cards.filter(c=>c._photoFile).map(c=>c._photoFile);
      ...
      const updateProgress=(i,percent)=>{
        loaded[i]=(cards[i]._photoFile?.size||0)*percent/100;
        const total=totalBytes?Math.floor(loaded.reduce((a,b)=>a+b,0)/totalBytes*100):100;
      ...
      try {
        ...
        for(let i=0;i<cards.length;i++){
          ...
          activeCard=cards[i];
          if(activeCard._existingPhoto){refs.push(activeCard._existingPhoto.id||activeCard._existingPhoto.fileId);activeCard=null;continue;}
          const file=activeCard._photoFile;updateProgress(i,0);
          status.textContent='Fotoğraf '+(i+1)+' / '+cards.length+' yükleniyor…';
          const ref=file._pzUpload||await this._uploadWebFile(file,'uploadStart',undefined,p=>updateProgress(i,p));file._pzUpload=ref;
          updateProgress(i,100);activeCard.querySelector('[data-photo-status]').textContent='Yükleme tamamlandı — doğrulanıyor';
          try{await this._webCall('uploadConfirm',ref);}catch(e){delete file._pzUpload;throw e;}
        ...
        const edit=form._editing;
        const payload={...values,currency:'EUR',priceEur:values.priceEur,stockQuantity:Number(values.stockQuantity),mediaIds:refs.concat(form._retainedVideos||[])};
        if(edit){payload.listingKey=edit.listingKey;payload.expectedUpdatedAt=edit.updatedAt;}
        else payload.idempotencyKey=this._webProductKey;
        const result=await this._webCall(edit?'updateProduct':'create',payload);
    ...
    _renderInventoryModal() {
      if (this._state.authUi === "active_supplier") return "";
